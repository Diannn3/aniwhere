...
create or replace function public.can_edit_place(target_place_id uuid)
returns boolean
language sql
stable
security definer
...
revoke all on function public.can_edit_place(uuid) from public;
...
on public.places for select
to authenticated
using (public.can_edit_place(id));
...
to authenticated
using (public.can_edit_place(id))
with check (public.can_edit_place(id));
...
to authenticated
using (public.can_edit_place(place_id))
with check (public.can_edit_place(place_id));
...
to authenticated
with check (
  public.can_edit_place(place_id)
