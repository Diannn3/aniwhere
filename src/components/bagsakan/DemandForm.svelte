...
<form onsubmit={save} novalidate class="space-y-6 rounded-xl border border-[#20251E]/15 bg-white p-4 sm:p-6">
  ...
  <div class="grid gap-4 sm:grid-cols-2">
    <div><label for="bag-demand-cropKey" class="mb-1 block font-semibold">{isFil ? 'Pananim' : 'Crop'} *</label><select id="bag-demand-cropKey" bind:value={cropKey} onchange={() => { duplicateId = null; errors = {}; }} aria-invalid={Boolean(errors.cropKey)} class="min-h-11 w-full rounded-lg border border-[#20251E]/30 bg-[#FFFDF8] px-3">{#each SUPPORTED_CROPS as crop}<option value={crop.key}>{isFil ? crop.labelFil : crop.labelEn}</option>{/each}<option value="other">{isFil ? 'Iba pa' : 'Other'}</option></select>{#if errors.cropKey}<p class="mt-1 text-sm text-[#6E3511]">{errors.cropKey}</p>{/if}{#if duplicateId}<button type="button" onclick={() => onEditExisting(duplicateId!)} class="mt-2 min-h-11 font-bold text-[#486320] underline">{isFil ? 'I-edit ang kasalukuyang pangangailangan' : 'Edit existing need'}</button>{/if}</div>
    {#if cropKey === 'other'}<div><label for="bag-demand-customCropLabel" class="mb-1 block font-semibold">{isFil ? 'Pangalan ng pananim' : 'Crop name'} *</label><input id="bag-demand-customCropLabel" bind:value={customCropLabel} aria-invalid={Boolean(errors.customCropLabel)} class="min-h-11 w-full rounded-lg border border-[#20251E]/30 px-3" />{#if errors.customCropLabel}<p class="mt-1 text-sm text-[#6E3511]">{errors.customCropLabel}</p>{/if}</div>{/if}
    <div><label for="bag-demand-maxKg" class="mb-1 block font-semibold">{isFil ? 'Pinakamaraming kg na matatanggap' : 'Maximum kg you can receive'}</label><input id="bag-demand-maxKg" type="number" min="0" step="any" bind:value={maxKgInput} disabled={unknownCapacity} aria-invalid={Boolean(errors.maxKg)} class="min-h-11 w-full rounded-lg border border-[#20251E]/30 px-3 tabular-nums disabled:bg-[#20251E]/5" /><label class="mt-2 flex min-h-11 items-center gap-2"><input id="bag-demand-unknown" type="checkbox" bind:checked={unknownCapacity} />{isFil ? 'Hindi pa alam ang kapasidad' : 'Capacity unknown'}</label>{#if errors.maxKg}<p class="mt-1 text-sm text-[#6E3511]">{errors.maxKg}</p>{/if}</div>
    <div><label for="bag-demand-pricePerKg" class="mb-1 block font-semibold">{isFil ? 'Halimbawang presyo, ₱/kg (opsyonal)' : 'Sample price, ₱/kg (optional)'}</label><input id="bag-demand-pricePerKg" type="number" min="0" step="any" bind:value={priceInput} aria-invalid={Boolean(errors.pricePerKg)} class="min-h-11 w-full rounded-lg border border-[#20251E]/30 px-3 tabular-nums" />{#if errors.pricePerKg}<p class="mt-1 text-sm text-[#6E3511]">{errors.pricePerKg}</p>{/if}</div>
    <div><label for="bag-demand-validFrom" class="mb-1 block font-semibold">{isFil ? 'Simula ng bisa' : 'Valid from'} *</label><input id="bag-demand-validFrom" type="date" bind:value={validFrom} aria-invalid={Boolean(errors.validFrom)} class="min-h-11 w-full rounded-lg border border-[#20251E]/30 px-3" />{#if errors.validFrom}<p class="mt-1 text-sm text-[#6E3511]">{errors.validFrom}</p>{/if}</div>
    <div><label for="bag-demand-validUntil" class="mb-1 block font-semibold">{isFil ? 'Hanggang kailan' : 'Valid until'} *</label><input id="bag-demand-validUntil" type="date" bind:value={validUntil} aria-invalid={Boolean(errors.validUntil)} class="min-h-11 w-full rounded-lg border border-[#20251E]/30 px-3" />{#if errors.validUntil}<p class="mt-1 text-sm text-[#6E3511]">{errors.validUntil}</p>{/if}</div>
    <div><label for="bag-demand-status" class="mb-1 block font-semibold">{isFil ? 'Katayuan' : 'Status'}</label><select id="bag-demand-status" bind:value={status} aria-invalid={Boolean(errors.status)} class="min-h-11 w-full rounded-lg border border-[#20251E]/30 px-3"><option value="active">{isFil ? 'Aktibong demo' : 'Active demo'}</option><option value="draft">{isFil ? 'Burador' : 'Draft'}</option>{#if demand}<option value="paused">{isFil ? 'Naka-pause' : 'Paused'}</option>{/if}</select>{#if errors.status}<p class="mt-1 text-sm text-[#6E3511]">{errors.status}</p>{/if}</div>
  </div>
