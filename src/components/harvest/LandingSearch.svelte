...
<form class="landing-search" onsubmit={continueToHarvest} role="search" aria-label={lang === 'fil' ? 'Hanapin ang ani' : 'Search by crop'}>
  ...
  <input id="landing-crop" bind:value={crop} oninput={() => (error = false)} aria-invalid={error} aria-describedby={error ? 'landing-crop-error' : undefined} maxlength="80" autocomplete="off" placeholder={lang === 'fil' ? 'Anong ani ang ibebenta mo?' : 'What are you harvesting?'} />
