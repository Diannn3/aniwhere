...
<div class="almanac-page comparison-page w-full min-w-0 max-w-none px-4 py-6 sm:px-8 sm:py-8 lg:px-12 xl:px-16">
  ...
  ? copy(
      `OpenRouteService road estimate from ${originMun.name.split(',')[0]} municipality center · ${formatEstimatedDriveDuration(route) ?? 'duration unavailable'} estimated drive · not live traffic`,
      `Tantya ng OpenRouteService mula sa sentro ng ${originMun.name.split(',')[0]} · ${formatEstimatedDriveDuration(route) ?? 'walang tantyang oras'} tantyang biyahe · hindi live traffic`,
    )
  : copy(
