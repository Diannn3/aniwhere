export const ANI_TOOL_DECLARATIONS = [
  ...
  { name: 'get_route_estimate', description: 'Get AniWhere\'s truthful distance context for one known outlet. Returns road distance/time only when a reviewed routing artifact exists; otherwise returns straight-line context from the municipality centroid.', parameters: { type: 'OBJECT', properties: { outletId: { type: 'STRING' } }, required: ['outletId'] } },
  {
    name: 'navigate_to',
    description: 'Navigate to an allowlisted AniWhere route. Discovery may optionally open the list or map and select one known outlet.',
    parameters: {
      type: 'OBJECT',
      properties: {
        path: { type: 'STRING' },
